...
@Component({
  ...
  styleUrls: ['./antecedent-family-list.component.scss'],
})
export class AntecedentFamilyListComponent implements OnInit {
  private subs = new SubSink();

  public searchForm: FormGroup;

  public antecedentFamille: IAntecedent;
  ...
  ngOnInit(): void {
    this.initform();
    this.getAntecedentFamille();
    const actGroup = {
      active: false,
      id: 0,
      name: 'ANTECEDENT FAMILLE',
    };
  ...
  public getAntecedentFamille() {
    this.showloading = true;
    this.subs.add(
      this.antecedentFamilleService.findAll(this.searchForm.value).subscribe(
        (response: PageList) => {
          console.log(response);
          this.showloading = false;
          this.currentPage = response.currentPage + 1;
  ...
  openUpdateForm(updateFormContent, item?) {
    this.antecedentFamille = item;
    console.log(this.antecedentFamille);
    this.modalService.open(updateFormContent, { size: 'lg' });
  }

  addAntecedentFamille() {
  ...
  rowSelected(antecedentFamille: IAntecedent, index: number) {
    this.currentIndex = index;
    this.antecedentFamille = antecedentFamille;
  }
}
