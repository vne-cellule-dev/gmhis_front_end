...
export class Sidebar2Component implements OnInit ,  OnDestroy{ private subs = new SubSink();
  ...
  private canAccessBasicFiles(){
    return  this.userService.checkAuthority(SideBar2MenuAuthorityEnum.ACCESS_BASIC_FILES);
   }
   ...
   private canAccessFreightForwarder(){
    return  this.userService.checkAuthority(FreightForwarderAuthorityEnum.FREIGHT_FORWARDER_LIST);
   }

   private canAccessContainerCost(){
    return  this.userService.checkAuthority(containerCostAuthorityEnum.CONTAINER_COST_LIST);
   }
   ...
   private canAccessStockOutReason(){
    return  this.userService.checkAuthority(StockOutReasonAuthorityEnum.STOCK_OUT_REASON_LIST);
   }
   ...
   private canAccessUserManagenent(){
    return  this.userService.checkAuthority(SideBar2MenuAuthorityEnum.ACCESS_USER_MANAGEMENT);
   }
   ...
   private canAccessEventLog(){
    return  this.userService.checkAuthority(SideBar2MenuAuthorityEnum.ACCESS_EVENT_LOG);
   }

   private canAccessApllicationSetting(){
    return  this.userService.checkAuthority(SideBar2MenuAuthorityEnum.ACCESS_APPLICATION_SETTINGS);
   }
